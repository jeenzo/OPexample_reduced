...
for n, c in tune_on:
    maxRep = replU
    name = "bm_un"
    filename = f"0_un_{n}_{c}.sh"
    with open(filename, "w") as f:
            f.write("#!/bin/bash\n\n")
            f.write(f"#SBATCH --job-name=0_un_{n}_{c}\n")
            f.write("#SBATCH --time=01:00:00\n")
            f.write("#SBATCH --ntasks=1\n")
            f.write("#SBATCH --cpus-per-task=1\n")
            f.write("#SBATCH --mem=8G\n")
            f.write("#SBATCH --mail-type=FAIL,END\n")
            f.write(f"#SBATCH --output=./out/0_un_{n}_{c}_%A_%a.out\n")
            f.write(f"#SBATCH --error=./err/0_un_{n}_{c}_%A_%a.err\n")
            f.write(f"#SBATCH --array=1-{maxRep}\n")
    ...
    name = "bm_cb"
    filename = f"0_cb_{n}_{c}.sh"
    with open(filename, "w") as f:
            f.write("#!/bin/bash\n\n")
            f.write(f"#SBATCH --job-name=c0_b_{n}_{c}\n")
            f.write("#SBATCH --time=02:00:00\n")
            f.write("#SBATCH --ntasks=1\n")
            f.write("#SBATCH --cpus-per-task=1\n")
            f.write("#SBATCH --mem=8G\n")
            f.write("#SBATCH --mail-type=FAIL,END\n")
            f.write(f"#SBATCH --output=./out/0_cb_{n}_{c}_%A_%a.out\n")
            f.write(f"#SBATCH --error=./err/0_cb_{n}_{c}_%A_%a.err\n")
            f.write(f"#SBATCH --array=1-{maxRep}\n")
...
with open(filename, "w") as f:
    ...
    for n, c in tune_on:
        ...
        f.write(f"sbatch ./slurm/0_un_{n}_{c}.sh\n") 
        f.write(f"sbatch ./slurm/0_cb_{n}_{c}.sh\n") 
