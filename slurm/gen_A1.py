...
for n, c in tune_on:
    ...
    with open(filename, "w") as f:
            ...
            f.write("# Run\n")
            if n > 200:
                f.write(f"./JOBASP-f --in {name} --N {n} --cap 6 --trueC {c} --repl $SLURM_ARRAY_TASK_ID\n")
            else:
                f.write(f"./JOBASP-f --in {name} --N {n} --cap {c} --repl $SLURM_ARRAY_TASK_ID\n")

    filename = f"long_eval_n{n}_c{c}.sh"
    with open(filename, "w") as f:
            f.write("#!/bin/bash\n\n")
            f.write(f"#SBATCH --job-name=l_{n}_{c}\n")
            f.write("#SBATCH --time=24:00:00\n")
            f.write("#SBATCH --ntasks=1\n")
            f.write("#SBATCH --cpus-per-task=1\n")
            f.write("#SBATCH --mem=24G\n")
            f.write("#SBATCH --mail-type=FAIL,END\n")
            f.write(f"#SBATCH --output=./out/l_{n}_{c}_%A_%a.out\n")
            f.write(f"#SBATCH --error=./err/l_{n}_{c}_%A_%a.err\n")
            f.write(f"#SBATCH --array=1-{maxRep}\n")
            ...
            f.write("# Run\n")
            if n > 200:
                f.write(f"./JOBASP-f --in {name} --N {n} --cap 6 --trueC {c} --repl $SLURM_ARRAY_TASK_ID --seg 2000\n")
            else:
                f.write(f"./JOBASP-f --in {name} --N {n} --cap {c} --repl $SLURM_ARRAY_TASK_ID --seg 2000\n")
...
with open(filename, "w") as f:
    ...
    for n, c in tune_on:
        ...
        f.write(f"sbatch ./slurm/long_eval_n{n}_c{c}.sh\n")
